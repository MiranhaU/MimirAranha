...
// Configuração do Firebase (idêntica à original)
...
function forceAttachLogoutButtonListener() {
  setInterval(() => {
    const logoutButton = document.getElementById('logoutButton');
    if (logoutButton && logoutButton.onclick !== handleLogout) {
      logoutButton.onclick = handleLogout;
      console.log("Logout: listener FORCE-atrib. [" + new Date().toLocaleTimeString() + "]");
    }
  }, 2000);
}

// Helper: conecta o botão de logout quando o DOM estiver pronto
function attachLogoutButtonListener() {
  const logoutButton = document.getElementById('logoutButton');
  if (logoutButton) {
    logoutButton.onclick = handleLogout; // Usar onclick garante que sobrescreve qualquer anterior
    console.log("Botão de logout conectado com sucesso.");
  } else {
    console.warn("Botão de logout (logoutButton) não encontrado no DOM.");
    // Tentar novamente em 500ms caso o DOM ainda esteja carregando
    setTimeout(attachLogoutButtonListener, 500);
  }
}

// Função para criar botão de salvamento manual
function createSaveButton() {
  // Verificar se o botão já existe para evitar duplicação
  if (document.getElementById('manualSaveButton')) return;
...
// Envolva tudo dentro de DOMContentLoaded para garantir que o DOM esteja pronto
document.addEventListener('DOMContentLoaded', () => {
  console.log("DOM carregado, inicializando autenticação...");
  
  // Iniciar o monitoramento forçado do botão de logout
  forceAttachLogoutButtonListener();
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      // Conectar o botão de logout de forma robusta
...
export { handleLogout, attachLogoutButtonListener };
